...
void desenha_bancos() {
    ...
    int fileiras = 10;
    int bancos_por_fileira = 3; // Seção esquerda, corredor, seção direita
...
void desenha_confessionarios() {
    // Confessionários nas laterais
    float base_y = ALTURA_PLATAFORMA;
    
    glColor3f(0.5f, 0.3f, 0.2f); // Madeira escura
    
    // Confessionário esquerdo
    glPushMatrix();
    glTranslatef(-18.0f, base_y + 1.2f, -25.0f);
    glScalef(3.0f, 2.4f, 2.0f);
    glutSolidCube(1.0f);
    glPopMatrix();
    
    // Confessionário direito
    glPushMatrix();
    glTranslatef(18.0f, base_y + 1.2f, -25.0f);
    glScalef(3.0f, 2.4f, 2.0f);
    glutSolidCube(1.0f);
